...
basedatos= conexion.fondos
coleccion= basedatos.lista
estilos={'todos':'active','animales':'','Naturaleza':'','ciudad':'','Deporte':'','personas':''}
...
def galeria():
    ...
    if t == None:
        ...
        l=coleccion.find({"tags": {"$in":[t]}})
        estilos[t]="active"  
    return render_template("index.html",activo=estilos,lista=l)
def mostrar():
    todos=coleccion.find()
    return render_template('index.html',activo=estilos, lista=todos)
...
def agregar_fondo():
    ...
    coleccion.insert({"fondo": archivo,"titulo":titu,"descripcion":desc,"tags":tags})
    return redirect("/")
